		...
		if pinner, ok := s.qriNode.Repo.Store().(cafs.Pinner); ok {
			...
			go func(cfg *config.Config, cfgPath string) {
				...
				if cfg != nil && cfg.Render != nil && cfg.Render.TemplateUpdateAddress != "" {
					if latest, err := lib.CheckVersion(context.Background(), node.Namesys, cfg.Render.TemplateUpdateAddress, cfg.Render.DefaultTemplateHash); err == lib.ErrUpdateRequired {
